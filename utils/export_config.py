...
class ExportConfig(QtWidgets.QDialog, Ui_ConfigsSaida):
    ...
    def finalizar(self):
        if not self.nome_input.text().strip() or not self.sensor_input.text().strip() or not self.unidade_input.text().strip():
            return
        else:
            if self.destino:
                self.destino = QtWidgets.QFileDialog.getExistingDirectory(
                    self, caption="Local para salvar o relatório de calibração", dir=self.destino
                )
            else:
                self.destino = QtWidgets.QFileDialog.getExistingDirectory(
